...
class MethodThread(Thread):
    ...
    def run(self):
        ...
        SESSION.i_curve_ids.clear()
        SESSION.e_curve_ids.clear()
        SESSION.i_values.clear()
        SESSION.e_values.clear()
...
def live_measurement_widget(method: experimental_BatteryCycling):
    ...
    if SESSION.thread:
        ...
        while SESSION.thread.is_alive():
            ...
            curve_no = len(SESSION.i_curve_ids)
            ...
            if SESSION.i_values:
                i_chart.altair_chart(make_chart2(values=SESSION.i_values))

            if SESSION.e_values:
                e_chart.altair_chart(make_chart2(values=SESSION.e_values))
...
def show_measurement_widget():
    ...
    if SESSION.i_curve_ids:
        c1.altair_chart(make_chart2(values=SESSION.i_values).interactive())

    if SESSION.e_curve_ids:
...
def make_chart2(values: list[dict[str, float]]) -> alt.Chart:
    """Make altair chart for curve id."""
    first_row = values[0]
    metadata = SESSION.curves_metadata[first_row['id']]

    x_label, y_label = metadata.labels
    x_unit, y_unit = metadata.units

    source = alt.Data(values=values)
