...
For the inheritance hierarchy of the the techniques, see xref:dotnet:api:core.adoc[].
...
* xref:#api-lsv[PalmSens.Techniques.LinearSweepVoltammetry]
* xref:#api-cv[PalmSens.Techniques.CyclicVoltammetry]
* xref:#api-fcv[PalmSens.Techniques.FastCyclicVoltammetry]
* xref:#api-acv[PalmSens.Techniques.ACVoltammetry]

**Pulsed Techniques**

* xref:#api-dpv[PalmSens.Techniques.DifferentialPulseVoltammetry]
* xref:#api-swv[PalmSens.Techniques.SquareWaveVoltammetry]
* xref:#api-npv[PalmSens.Techniques.NormalPulseVoltammetry]

**Amperometric Techniques**

* xref:#api-ca[PalmSens.Techniques.ChronoAmperometry]
* xref:#api-msa[PalmSens.Techniques.MultiStepAmperometry]
* xref:#api-fam[PalmSens.Techniques.FastAmperometry]
* xref:#api-pad[PalmSens.Techniques.PulsedAmperometricDetection]
* xref:#api-mpad[PalmSens.Techniques.MultiplePulseAmperometry]

**Potentiometric Techniques**

* xref:#api-ocp[PalmSens.Techniques.OpenCircuitPotentiometry]
* xref:#api-pot[PalmSens.Techniques.ChronoPotentiometry]
* xref:#api-lsp[PalmSens.Techniques.LinearSweepPotentiometry]
* xref:#api-mp[PalmSens.Techniques.MultiStepPotentiometry]
* xref:#api-scp[PalmSens.Techniques.StrippingChronoPotentiometry]

**Coulometric techniques**

* xref:#api-cc[PalmSens.Techniques.ChronoCoulometry]

**Other**

* xref:#api-eis[PalmSens.Techniques.ElectrochemicalImpedanceSpectroscopy]
* xref:#api-fis[PalmSens.Techniques.FastImpedanceSpectroscopy]
* xref:#api-gis[PalmSens.Techniques.GalvanostaticImpedanceSpectroscopy]
* xref:#api-fgis[PalmSens.Techniques.FastGalvanostaticImpedanceSpectroscopy]
* xref:#api-mm[PalmSens.Techniques.MixedMode]
* xref:#api-ms[PalmSens.Techniques.MethodScript]

Please refer to the https://www.palmsens.com/knowledgebase-article/pstrace-user-manual/[PSTrace manual] for explanations and expected values for each parameter.
...
* Short name: MA
* Class: `PalmSens.Techniques.MultistepAmperometry`
* `TechniqueID`: 12
* `MethodID`: `"ma"`

Create multi-step potentiometry method by specifying levels using `PalmSens.Techniques.ELevel`.

----
method = PalmSens.Techniques.MultistepAmperometry();
level = PalmSens.Techniques.ELevel();
level.Duration = 2.0;
level.Level = 1.0;
...
|`UseSelectiveRecord`
|If True, enable record toggles for each level
|`System.Bool`

|`UseLimits`
|If True, enable limit toggles for each level
|`System.Bool`
|===

=== ELevel

* Class: `PalmSens.Techniques.ELevel`
...
|`UseMaxLimit`
|If True, continue with next level when `MaxLimit` is reached
|`System.Float`
...
|`UseMinLimit`
|If True, continue with next level when `MinLimit` is reached
|`System.Float`
...
* Short name: LSP
* Class: `PalmSens.Techniques.LinearSweepPotentiometry`
* `TechniqueID`: 17
* `MethodID`: `"lsp"`
...
The applicable range depends on the value of `StepCurrent` since the data
...
* Short name: MP
* Class: `PalmSens.Techniques.MultistepPotentiometry`
* `TechniqueID`: 13
* `MethodID`: `"mp"`
...
* Short name: CC
* Class: `PalmSens.Techniques.ChronoCoulometry`
* `TechniqueID`: n/a
* `MethodID`: `"cc"`
...
For example, using the default properties `ScanType = Impedance`,
`enumScanType.FixedPotential` and `FreqType = Impedance.enumFrequencyType.Scan`.
...
* Short name: GIS
* Class: `PalmSens.Techniques.ImpedimetricGstatMethod`
* `TechniqueID`: 14
* `MethodID`: `"gis"`

The galvanostatic impedance method is similar to <<Electrochemical Impedance Spectroscopy>>.
See the documentation for that class for more information how to set up the method.
...
|`ScanType`
|Scan type specifies whether a single or multiple frequency scans are performed.
|`Impedance.enumScanType`

|`FreqType`
|Frequency type specifies whether to perform a scan on a range of frequencies or to measure a single frequency.
|`Impedance.enumFrequencyType`
...
* Short name: MIX
* Class: `PalmSens.Techniques.MixedMode`
* `TechniqueID`: 15
* `MethodID`: `"mm"`

Use `MixedMode.AddStage(EnumMixedModeStageType stage)` to add a stage, e.g.:

----
method = MixedMode()
newStage = method.AddStage(MixedMode.EnumMixedModeStageType.ConstantE);
newSTage.Potential = 1.0;
----
...
* Class: `PalmSens.Techniques.MixedModeStageConstantE`

Amperometric detection stage. Apply constant potential during this stage.
...
* Class: `PalmSens.Techniques.MixedModeStageConstantI`

Potentiometry stage. Apply constant fixed current during this stage.
...
* Class: `PalmSens.Techniques.MixedModeStageSweepE`

Linear sweep detection stage.

Ramps the voltage from `BeginPotential` to `EndPotential` during this stage.
...
The applicable range depends on the value of `StepPotential`
...
* Class: `PalmSens.Techniques.MixedModeStageOpenCircuit`

Open Circuit stage. Measure the open circuit potential during this stage.
...
* Class: `PalmSens.Techniques.MixedModeStageImpedance`

Electostatic impedance stage. This is like EIS with a single frequency step
...
on your instrument. See xref:dotnet:ROOT:methodscript.adoc[this page] for more information.

* Short name: MS
* Class: `PalmSens.Techniques.MethodScriptSandbox`
* `TechniqueID`: 19
* `MethodID`: `"ms"`
...
dotnet:ROOT:auxilary_input.png[Change auxilary input]
...
[,csharp]
----
method = new AmperometricDetection();
method.MuxMethod = MuxMethod.None;
method.MuxMethod = MuxMethod.Alternatingly;
method.MuxMethod = MuxMethod.Sequentially;
----

=== Sequential settings

This example shows how to create a method to measure sequentially on channels 1, 3, and 5.

[,csharp]
----
method = new AmperometricDetection();  <1>

method.MuxMethod = MuxMethod.Sequentially;  <2>

// Measure sequentially on channel 1, 3 and 5
method.UseMuxChannel = new bool[] {  <3>
    true, true, false, false, false, false, false, true
};
----
<1> Create a new chronoamperometry method with the default settings
<2> Set sequential mux method
<3> Create an array of booleans indicating on which channels you would like to measure

For a 16 channel mux you can create a bool array for 16 channels.

=== Alternating settings

Note that only Chronoamperometry (Amperometric Detection) and (Open Circuit) Potentiometry support alternating measurements.
Alternating multiplexer measurement can only measure on successive channels and must start with the first channel.
For example, it is possible to alternatingly measure on channels 1 through 4 but it is not possible to alternatingly measure on channel 1, 3 and 5.

[,csharp]
----
method = new AmperometricDetection();  <1>

method.MuxMethod = MuxMethod.Alternatingly;  <2>

// Measure sequentially on channel 1, 2, and 3
method.UseMuxChannel = new bool[] {  <3>
    true, true, true, false, false, false, false, false
};
----
<1> Create a new chronoamperometry method with the default settings
<2> Set alternating mux method
<3> Create an array of booleans indicating on which channels you would like to measure

The code above will perform a measurement on the first two and last channels of an 8-channel multiplexer.
For a 16-channel multiplexer you would need to define a boolean array with 16 members.
