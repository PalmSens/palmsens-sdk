...
figure(2);
subplot 311;
plot(measurement.curves(1).xData, measurement.curves(1).yData, '*r', result.FitCurves(1).xData, result.FitCurves(1).yData);
title('Nyquist');
xlabel(measurement.curves(1).xUnit);
ylabel(measurement.curves(1).yUnit);
legend(measurement.name, 'fit');
% Plot the Bode plot impedance over frequency
figure(2);
subplot 312;
plot(log10(measurement.curves(2).xData), log10(measurement.curves(2).yData), '*r', log10(result.FitCurves(2).xData), log10(result.FitCurves(2).yData));
title('Bode (Z over frequency)');
xlabel(['Log ' measurement.curves(2).xUnit]);
ylabel(['Log ' measurement.curves(2).yUnit]);
legend(measurement.name, 'fit');
% Plot the Bode plot -phase over frequency
figure(2);
subplot 313;
plot(log10(measurement.curves(3).xData), measurement.curves(3).yData, '*r', log10(result.FitCurves(3).xData), result.FitCurves(3).yData);
title('Bode (-phase over frequency)');
xlabel(['Log ' measurement.curves(3).xUnit]);
ylabel(measurement.curves(3).yUnit);
legend(measurement.name, 'fit');
