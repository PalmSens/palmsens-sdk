...
function plotDataOverlay(hObject, measurement1, measurement2)
    ...
    % must be set to on.
    plot(x1, y1, x2, y2);
    title([measurement1.type ' & ' measurement2.type]);
    xlabel(curve1.xUnit);
    ylabel(curve1.yUnit);
    legend(names);
    if handles.Settings.Plot.axesEqual == 1
...
function plotDataRealtime(hObject)
    ...
    % must be set to on.
    plot(x, y);
    title(char(measurement.Title));
    xlabel(char(curves(1).XUnit.ToString()));
    ylabel(char(curves(1).YUnit.ToString()));
    if handles.Settings.Plot.axesEqual == 1
        axis equal;
    else
        axis auto;
...
function plotEISDataRealtime(hObject)
    ...
    plot(x, y);
    title(char(measurement.Title));
    xlabel(['ZRe(' char(ZRe.Unit.ToString()) ')']);
    ylabel(['ZIm(' char(ZIm.Unit.ToString()) ')']);
    if handles.Settings.Plot.axesEqual == 1
        axis equal;
    else
        axis auto;
