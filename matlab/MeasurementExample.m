...
disp('');
disp([newline 'Attempting to open a connection with ' deviceName]);
...
elseif strcmp(measurement.type, 'Impedance Spectroscopy') || strcmp(measurement.type, 'Galvanostatic Impedance Spectroscopy') % Check if Impedance Spectroscopy
    ...
    figure(1);
    subplot 311;
    plot(measurement.curves(1).xData, measurement.curves(1).yData);
    title('Nyquist');
    xlabel(measurement.curves(1).xUnit);
    ylabel(measurement.curves(1).yUnit);
    legend(measurement.name);
    % Plot the Bode plot impedance over frequency
    figure(1);
    subplot 312;
    plot(measurement.curves(2).xData, measurement.curves(2).yData);
    title('Bode (Z over frequency)');
    xlabel(measurement.curves(2).xUnit);
    ylabel(measurement.curves(2).yUnit);
    legend(measurement.name);
    % Plot the Bode plot -phase over frequency
    figure(1);
    subplot 313;
    plot(measurement.curves(3).xData, measurement.curves(3).yData);
    title('Bode (-phase over frequency)');
    xlabel(measurement.curves(3).xUnit);
