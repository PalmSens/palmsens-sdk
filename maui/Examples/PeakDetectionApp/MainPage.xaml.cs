...
namespace PalmSensPeakDetection
{
    public partial class MainPage : ContentPage
    {
        ...
        private void PeakDetectProgress_ProgressChanged(object sender, PeakDetectProgressUpdate e)
        {
            switch (e.Progress)
            {
                case EnumPeakDetectProgress.Started:
                    Log.Add($"Peak detection started.");
                    Log.Add($"{e.NCurves} curves remaining.");
                    break;
                case EnumPeakDetectProgress.PeakDetected:
                    Log.Add($"{e.NPeaksFound} peaks found.");
                    break;
                case EnumPeakDetectProgress.ProcessingCurve:
                    Log.Add($"Detecting peaks in {e.CurveTitle}.");
                    break;
                case EnumPeakDetectProgress.CurveProcessed:
                    Log.Add($"{e.NRemainingCurves} curves remaining.");
                    break;
                case EnumPeakDetectProgress.Cancelled:
                    break;
                case EnumPeakDetectProgress.Finished:
                    Log.Add("Completed detecting peaks.");
                    plotView1.CorePlot.UpdateSimpleCurvesPeaks(_activeMeasurement.SimpleCurveCollection);
                    break;
            }
        }
        ...
        {
            DetectPeaksBtn.IsEnabled = _activeMeasurement.SimpleCurveCollection.Count > 0; ;
