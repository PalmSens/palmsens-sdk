...
The simplest way to run an expirement is to use the `ps.measure()` function.
...
`ps.connect()` returns an instance of `InstrumentManager()`, which can be used to control the instrument and start a measurement:
...
By default, `connect()` connects to any plugged-in USB instrument it discovers.
It gives an error when multiple instruments are discovered.
With more instruments connected, you can use `ps.discover()` to find all devices and manage them yourself.
