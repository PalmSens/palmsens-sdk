...
>>> cv = ps.CyclicVoltammetry(
...     current_range = {'min':'cr_10_mA', 'max':'cr_1_uA', 'start':'cr_100_uA'}
... )
>>> cv.current_range
CurrentRange(max=<CURRENT_RANGE.cr_10_mA: 8>, min=<CURRENT_RANGE.cr_1_uA: 4>, start=<CURRENT_RANGE.cr_100_uA: 6>)
----
