...
`psCommSimpleWinForms` is a reference to the instance of the `psCommSimpleWinForms` component.

----
psCommSimpleWinForms.MeasurementStarted += PsCommSimpleWinForms_MeasurementStarted; <1>

psCommSimpleWinForms.MeasurementEnded += PsCommSimpleWinForms_MeasurementEnded; <2>

psCommSimpleWinForms.SimpleCurveStartReceivingData += PsCommSimpleWinForms_SimpleCurveStartReceivingData; <3>
----
...
Optionally, when using a multiplexer the channel can be specified as an integer, for example `psCommSimpleWinForms.Measure(method, 2)`.
...
----
SimpleMeasurement activeSimpleMeasurement = psCommSimpleWinForms.Measure(method);
----
...
During a measurement the property `psCommSimpleWinForms.DeviceState` property equals either `CommManager.DeviceState.Pretreatment` or `CommManager.DeviceState.Measurement`.
...
----
psCommSimpleWinForms.Disconnect();
// or
psCommSimpleWinForms.Dispose();
----

The `psCommSimpleWinForms.Disconnected` event is raised when the device is disconnected.
...
These issues typically arise when a timer is used, when using multiple threads, and when invoking commands in a callback on one on the `psCommSimpleWinForms`/`psMultiCommSimpleWinForms` events.

When using the simplified core wrapper, communication issues are prevented as much as possible.
Using commands to control the device from your `psCommSimpleWinForms`/`psMultiCommSimpleWinForms` event callbacks is blocked, to prevent communication issues.
...
This can be adjusted in the `PSCommSimpleWinForms.cs` or `PSMultiCommSimpleWinForms.cs` files in the `PalmSens.Core.Simplified` project.
