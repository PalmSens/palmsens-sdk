...
`psCommSimpleWPF` is a reference to the instance of the `psCommSimpleWPF` component.

----
psCommSimpleWPF.MeasurementStarted += PsCommSimpleWPF_MeasurementStarted; <1>

psCommSimpleWPF.MeasurementEnded += PsCommSimpleWPF_MeasurementEnded; <2>

psCommSimpleWPF.SimpleCurveStartReceivingData += PsCommSimpleWPF_SimpleCurveStartReceivingData; <3>
----
...
Optionally, when using a multiplexer the channel can be specified as an integer, for example `psCommSimpleWPF.Measure(method, 2)`.
...
// or
psCommSimpleWPF.Dispose();
----

The `psCommSimpleWPF.Disconnected` event is raised when the device is disconnected.
...
These issues typically arise when a timer is used, when using multiple threads, and when invoking commands in a callback on one on the `psCommSimpleWPF`/`psMultiCommSimpleWPF` events.

When using the simplified core wrapper, communication issues are prevented as much as possible.
Using commands to control the device from your `psCommSimpleWPF`/`psMultiCommSimpleWPF` event callbacks is blocked, to prevent communication issues.
...
This can be adjusted in the `PSCommSimpleWPF.cs` or `PSMultiCommSimpleWPF.cs` files in the `PalmSens.Core.Simplified` project.
